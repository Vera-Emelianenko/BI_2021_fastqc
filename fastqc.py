...
def plot_gc_content(df):
    ...
    fig.savefig(os.path.join(args.outdir, "per_sequence_gc_content.png"), format='png', dpi=1000)
...
def plot_sequence_content(df):
    ...
    fig.savefig(os.path.join(args.outdir, "per_base_sequence_content.png"), format='png', dpi=1000)
...
def plot_n_content(df):
    ...
    fig.savefig(os.path.join(args.outdir, "per_base_n_content.png"), format='png', dpi=1000)
...
def per_sequence_quality_score():
    ...
    fg, ax = plt.subplots()
    ax.plot(x = 'Quality', y = 'Counts', data = quality_counts, label="Average quality per read")
    ax.xaxis.set_minor_locator(AutoMinorLocator(4))
    ax.yaxis.set_minor_locator(AutoMinorLocator(4))
...
def main():
    ...
    get_seq_quality(input_fastq_list=input_list)
    per_sequence_quality_score()
    per_base_sequence_quality()
    get_tile(input_fastq_list=input_list)
    per_tile_quality()
